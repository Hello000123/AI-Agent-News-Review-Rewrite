...
describe("Grok client", () => {
  ...
  it("sends a Grok 4.5 high-reasoning streaming request without leaking the key", async () => {
    const fetchMock = vi.fn().mockResolvedValue(streamingResponse());

    const content = await requestGrokCompletion(reviewRequest, {
      config,
  ...
  it("supports the official non-streaming schema and uses only final content", async () => {
    ...
    const content = await requestGrokCompletion(
  ...
  it("uses the validated Grok model selection instead of a stale config value", async () => {
    const fetchMock = vi.fn().mockResolvedValue(completionResponse("Final answer."));

    await requestGrokCompletion(
      { ...reviewRequest, model: "grok-4.5" },
      {
  ...
  it("fails before making a request when the API key is absent", async () => {
    ...
    await expect(
      requestGrokCompletion(reviewRequest, {
  ...
  it("captures a safe invalid-model cause and upstream HTTP status", async () => {
    ...
    let failure: unknown;
    try {
      await requestGrokCompletion(reviewRequest, {
  ...
  it("maps authentication and rate-limit failures to safe diagnostics", async () => {
    for (const [status, code, retryable] of [
      [401, "XAI_AUTH_ERROR", false],
      [429, "XAI_RATE_LIMIT", true],
    ] as const) {
      const fetchMock = vi.fn().mockResolvedValue(
  ...
  it("aborts and reports a timeout with workflow diagnostics", async () => {
    ...
    const requestPromise = requestGrokCompletion(reviewRequest, {
